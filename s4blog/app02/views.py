...
def menu(request):
    ...
    role_list = models.Role.objects.filter(users__user=user)    # 获得用户对应的所有角色
   ...
   """
    ...
    for per in permission_list:
        if not per['permission__menu_id']:  # 权限中是否有菜单
            continue
            # 将权限列表转换成字典格式，并重写组装
        item = {
            ...
            'status': True, # 显示该菜单下的子菜单
            'opened': False
        }
        # if re.match(per['permission__url'],"/jieke.html"):
        if re.match(per['permission__url'], request.path_info): # 判断访问的url与菜单栏中对应的url是否相同
            item['opened'] = True   # 该菜单栏打开状态
        pid = item['parent_id']

        all_menu_dict[pid]['child'].append(item)    # 通过菜单id将权限挂靠到对应的菜单上
